...
def allocate_buffers(engine):
    ...
    for i in range(engine.num_bindings):
        ...
        entry = {"name": name, "host": host_mem, "device": device_mem, "shape": tuple(shape)}  # เพิ่ม shape
...
def trt_infer(context, inputs, outputs, bindings, stream, blob):
    ...
    context.execute_async_v2(bindings=bindings, stream_handle=stream.handle)  # แก้ตรงนี้
    for out in outputs:
...
def run(source, engine_path: str, save_path: str = None,
    ...
    while True:
        ...
        cv2.putText(frame, f"Inference: {ms:.1f}ms | Objects: {len(results)}",
