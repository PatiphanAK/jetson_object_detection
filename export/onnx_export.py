...
print(
    "[2] Exporting yolo26n.pt -> ONNX  (opset=12, end2end=True, batch=5, dynamic=False)"
)
...
for node in m.graph.node:
    ...
    if a_dtype in INT_TYPES:
        new_nodes.append(
            helper.make_node("Div", [a, b], [f"{base}_q"], name=f"{base}_div")
        )
        new_nodes.append(
            helper.make_node("Mul", [f"{base}_q", b], [f"{base}_p"], name=f"{base}_mul")
        )
        new_nodes.append(
            helper.make_node("Sub", [a, f"{base}_p"], [out], name=f"{base}_sub")
        )
        n_mod_int += 1
    else:
        new_nodes.append(
        ...
        new_nodes.append(
            helper.make_node(
                "Floor", [f"{base}_q"], [f"{base}_f"], name=f"{base}_floor"
            )
        )
        new_nodes.append(
            helper.make_node("Mul", [f"{base}_f", b], [f"{base}_p"], name=f"{base}_mul")
        )
        new_nodes.append(
            helper.make_node("Sub", [a, f"{base}_p"], [out], name=f"{base}_sub")
        )
...
orig_dims = [
    ...
    for d in orig.type.tensor_type.shape.dim
]
